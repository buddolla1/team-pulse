...
const getAssignableRoleNames = async (adminRoleId) => {
  if (!adminRoleId) {
    return [];
  }

  const [roles] = await db.query('SELECT name FROM roles WHERE id = ?', [adminRoleId]);
  if (roles.length === 0) {
    return [];
  }

  return ROLE_CREATE_HIERARCHY[roles[0].name] || [];
};

const canAssignRole = async (adminRoleId, targetRoleId) => {
  if (!targetRoleId) {
    return true;
  }

  const [targetRoles] = await db.query('SELECT name FROM roles WHERE id = ?', [targetRoleId]);
  if (targetRoles.length === 0) {
    return false;
  }

  const assignableRoleNames = await getAssignableRoleNames(adminRoleId);
  return assignableRoleNames.includes(targetRoles[0].name);
};
...
const createAdmin = async (req, res) => {
  try {
    ...
    // Validate role_id if provided and ensure the creator can assign it
    if (role_id) {
      ...
      const allowed = await canAssignRole(req.admin?.role_id, role_id);
      if (!allowed) {
...
const getAuditLogs = async (req, res) => {
  try {
    const { page = 1, limit = 50, admin_id, action, entity_type, search } = req.query;
    const offset = (page - 1) * limit;
    ...
    if (searchTerm) {
      countQuery += ` AND (
        ...
        OR admin_id IN (
          SELECT id FROM admin_users WHERE username LIKE ? OR full_name LIKE ?
        )
      )`;
      const likeTerm = `%${searchTerm}%`;
