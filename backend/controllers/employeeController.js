...
const createEmployee = async (req, res) => {
  ...
  try {
    ...
    const normalizedJoiningDate = toDateOnly(joining_date);
    const normalizedLastWorkingDay = toDateOnly(last_working_day);
    const normalizedVisaStartDate = toDateOnly(current_visa_start_date);
    ...
    const computedVisaStatus = calculateVisaStatus(visa_type || 'None', normalizedVisaStartDate, normalizedVisaEndDate);
    ...
    const employeeRecord = {
      ...
      possible_candidate: possible_candidate || null,
      asset_id: asset_id || null,
      asset_return_id: asset_return_id || null,
      comments: comments || null,
      attrition: attrition || 'No',
      notice_period_days: notice_period_days || null,
      work_location: work_location || 'Onsite',
      temp_offshore_manager_id: temp_offshore_manager_id || null,
      temp_onsite_manager_id: temp_onsite_manager_id || null,
      visa_type: visa_type || 'None',
      visa_status: computedVisaStatus,
      current_visa_start_date: normalizedVisaStartDate,
      current_visa_end_date: normalizedVisaEndDate,
      i94_expiry_date: normalizedI94ExpiryDate,
      passport_number: passport_number || null,
      passport_expiry_date: normalizedPassportExpiryDate,
      sponsor_company: sponsor_company || null,
      visa_notes: visa_notes || null,
      password_hash: DEFAULT_EMPLOYEE_PASSWORD_HASH,
...
// Reset employee password to the standard temporary password
const resetEmployeePassword = async (req, res) => {
  ...
  try {
    ...
    const [employees] = await connection.query(
      'SELECT id, name, sso FROM employees WHERE id = ?',
      [id]
    );
    ...
    await connection.query(
      `UPDATE employees
       ...
       WHERE id = ?`,
      [DEFAULT_EMPLOYEE_PASSWORD_HASH, id]
    );
    ...
    if (req.admin) {
      await connection.query(
        ...
        [
          ...
          'employees',
          employee.id,
          `Reset employee password: ${employee.name} (${employee.sso || 'N/A'})`,
          req.ip || req.connection.remoteAddress,
          req.headers['user-agent'] || 'Unknown'
        ]
      );
    ...
    res.json({
      success: true,
      message: 'Employee password reset to Temp@1234',
      data: {
        id: employee.id,
        name: employee.name,
